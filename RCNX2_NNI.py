...
- Github: `https://github.com/ssrp/Multi-level-DCNet`
...
from capslayers import CapsuleLayer, Mask, Mask_CID
from keras.layers.normalization import BatchNormalization
from keras.preprocessing.image import ImageDataGenerator
from resnext_keras import __create_res_next as ResNext


def level(input_shape, n_class, routings, hparam=None):
    """     Hyper Parameter Settings      """
    if hparam is None:
        raise ValueError("Hyper Parameter Required! This file only for Hyper parameter Tuning!")
    ...
    """----------------------------------------------"""
    """    Primary Capsule Layer Hyper parameter     """
    """----------------------------------------------"""
    dim_capsule = [hparam['dim_capsule1'], hparam['dim_capsule2'], hparam['dim_capsule3']]
    n_channels = [hparam['n_channels1'], hparam['n_channels2'], hparam['n_channels3']]
    kernel_size = [hparam['kernel_size1'], hparam['kernel_size2'], hparam['kernel_size3']]
    strides = [hparam['strides1'], hparam['strides2'], hparam['strides3']]

    """----------------------------------------------"""
    """    Capsule Layer Hyper parameter     """
    """----------------------------------------------"""
    dim_capsule_c = [hparam['C_dim_capsule1'], hparam['C_dim_capsule2'], hparam['C_dim_capsule3'], hparam['C_dim_capsule4']]
    routings = [hparam['routings1'], hparam['routings2'], hparam['routings3'], hparam['routings4']]
    ...
    primarycaps_1, PrimaryCapsConv2D = PrimaryCap(ResBlockOutput, dim_capsule=dim_capsule[0], n_channels=n_channels[0], kernel_size=kernel_size[0],
                                                  strides=strides[0], padding='valid')

    # Applying ReLU Activation to primary capsules 
    conv = layers.Activation(activation[0])(PrimaryCapsConv2D)
    ...
    primarycaps_2, PrimaryCapsConv2D = PrimaryCap(ResBlockOutput, dim_capsule=dim_capsule[1], n_channels=n_channels[1], kernel_size=kernel_size[1],
                                                  strides=strides[1], padding='valid')

    # Applying ReLU Activation to primary capsules 
    conv = layers.Activation(activation[1])(PrimaryCapsConv2D)
    ...
    primarycaps_3, PrimaryCapsConv2D = PrimaryCap(ResBlockOutput, dim_capsule=dim_capsule[2], n_channels=n_channels[2], kernel_size=kernel_size[2],
                                                  strides=strides[2], padding='valid')
    ...
    # Merged DigitCaps
    digitcaps_0 = CapsuleLayer(num_capsule=n_class, dim_capsule=dim_capsule_c[0], routings=routings[0], channels=0,
                               name='digitcaps0')(mergedLayer)
    out_caps_0 = Length(name='capsnet_0')(digitcaps_0)

    # First Level DigitCaps
    digitcaps_1 = CapsuleLayer(num_capsule=n_class, dim_capsule=dim_capsule_c[1], routings=routings[1], channels=0,
                               name='digitcaps1')(primarycaps_1)
    out_caps_1 = Length(name='capsnet_1')(digitcaps_1)

    # Second Level DigitCaps
    digitcaps_2 = CapsuleLayer(num_capsule=n_class, dim_capsule=dim_capsule_c[2], routings=routings[2], channels=0,
                               name='digitcaps2')(primarycaps_2)
    out_caps_2 = Length(name='capsnet_2')(digitcaps_2)

    # Third Level DigitCaps
    digitcaps_3 = CapsuleLayer(num_capsule=n_class, dim_capsule=dim_capsule_c[3], routings=routings[3], channels=0,
...
def train(model, data, args, hparam):
    """
    Hyper Parameter settings
    """
    ...
    class SendMetrics(callbacks.Callback):
    ...
    def train_generator(x, y, batch_size, shift_fraction=0.):
        ...
        while 1:
            ...
            yield ([x_batch, y_batch], [y_batch, y_batch, y_batch, y_batch, x_batch[:, :, :, :]])
    ...
    model.fit_generator(generator=train_generator(x_train, y_train, args.batch_size, args.shift_fraction),
                        ...
                        validation_data=[[x_test, y_test],
                                         [y_test, y_test, y_test, y_test, x_test[:, :, :, :]]],
                        callbacks=[lr_decay, SendMetrics()])
...
def test(model, data, args):
    ...
    print('Test Accuracy (Merged DigitCaps): ',
          100.0 * np.sum(np.argmax(y_pred0, 1) == np.argmax(y_test, 1)) / (1.0 * y_test.shape[0]))

    print('Test Accuracy (Level 1 DigitCaps): ',
          100.0 * np.sum(np.argmax(y_pred1, 1) == np.argmax(y_test, 1)) / (1.0 * y_test.shape[0]))

    print('Test Accuracy (Level 2 DigitCaps): ',
          100.0 * np.sum(np.argmax(y_pred2, 1) == np.argmax(y_test, 1)) / (1.0 * y_test.shape[0]))

    print('Test Accuracy (Level 3 DigitCaps): ',
          100.0 * np.sum(np.argmax(y_pred3, 1) == np.argmax(y_test, 1)) / (1.0 * y_test.shape[0]))
    return 100.0 * np.sum(np.argmax(y_pred, 1) == np.argmax(y_test, 1)) / (1.0 * y_test.shape[0])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="3-level RCNX2 on CIFAR-10.")
    ...
    model.summary()
    trainable_count = int(
        np.sum([K.count_params(p) for p in set(eval_model.trainable_weights)]))
    # train or test
    if args.weights is not None:  # init the model weights with provided one
        model.load_weights(args.weights)
    if not args.testing:
        train(model=model, data=((x_train, y_train), (x_test, y_test)), args=args,hparam=params)
        args.weights = args.save_dir + '/trained_model.h5'
        accuracy = test(model=eval_model, data=(x_test, y_test), args=args)
        nni.report_intermediate_result(trainable_count)
